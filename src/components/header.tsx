...
export default function Header(props: { handleDifficulty: (nextDifficulty: Difficulties) => void }) {
    const [showDifficulties, setShowDifficulties] = useState<boolean>();

    return <header className='flex justify-between border-b-1 px-4'>
        <h1 className='text-3xl text-correct py-2'>DOUBLETS</h1>
        <div className='flex justify-between gap-x-2'>
            <button className={'w-12 p-2 ' + (showDifficulties ? 'bg-inactive-border' : 'hover:bg-inactive-border')} onClick={() => setShowDifficulties(!showDifficulties)}>
                <svg
            ...
            {showDifficulties ? <ul className="absolute top-13">
                <li>
                    <button className='border border-white w-24 cursor-pointer p-y-1 hover:bg-inactive-border' onClick={() => props.handleDifficulty(Difficulties.EASY)}>
                        Easy
                    </button>
                </li>
                <li>
                    <button className='border border-white w-24 cursor-pointer p-y-1 hover:bg-inactive-border' onClick={() => props.handleDifficulty(Difficulties.MEDIUM)}>
                        Medium
                    </button>
                </li>
                <li>
                    <button className='border border-white w-24 cursor-pointer p-y-1 hover:bg-inactive-border' onClick={() => props.handleDifficulty(Difficulties.HARD)}>
                        Hard
                    </button>
                </li>
            </ul> : null}
            <button className='w-12 p-2 hover:bg-inactive-border'>
                <svg
                    xmlns='http://www.w3.org/2000/svg'
                    viewBox='0 0 640 640'
                >
