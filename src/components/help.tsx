export default function Help(props: { showHelp: boolean, setShowHelp: React.Dispatch<React.SetStateAction<boolean>> }) {
    return props.showHelp ? <div className="fixed top-5 left-1/2 bg-grey-very-dark -translate-x-1/2 p-4 z-99 text-white rounded-sm">
        <div className="flex justify-between">
            <h2 className="font-(family-name:--start-fonts) font-bold">How to Play</h2>
            <button className="w-4 cursor-pointer -mt-2" onClick={() => props.setShowHelp(!props.showHelp)}>
        ...
        <p className="font-(family-name:--start-fonts)">Connect the start and end words using similar words.</p>
        <ul className="font-(family-name:--game-fonts)">
            <li>
                &#8226;
